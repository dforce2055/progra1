...
int main(){
	char matriz[10][ANCHO] = {{'\0'}, {'\0'}}, matrizbackup[10][ANCHO] = {{'\0'}, {'\0'}};
...
void leerMatriz(FILE *archivoEntrada, char matriz[][ANCHO], int alto){
	void eliminarcomas(char cadena[]);
	int i = 0;
	char cadena[24];
	
	while(!feof(archivoEntrada) && i < alto){
		fgets(cadena, 24,archivoEntrada);
		eliminarcomas(cadena);
		strcpy(matriz[i], cadena);
		i++;
		}
	}
//Elimina las comas de una cadena de caracteres
void eliminarcomas(char cadena[]){
	int i,j, largo;
	largo = strlen(cadena);
