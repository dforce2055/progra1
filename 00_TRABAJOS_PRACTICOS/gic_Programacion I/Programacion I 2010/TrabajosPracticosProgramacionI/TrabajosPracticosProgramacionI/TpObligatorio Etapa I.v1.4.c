...
int altaProducto(sproducto l[], int cant)
{
	...
	while((i<MAX) && (salida==1))
	{
		do
		{
			...
			eliminarRetornoCadena(l[i].codigoBarras);
		...
		else
		{
			...
			eliminarRetornoCadena(l[i].descripcion);
			if(strlen(l[i].descripcion)==0)
			{
				printf("Se cancela la carga del producto\n");
				return cant;
			}
			else
			{
				...
				eliminarRetornoCadena(l[i].nombreArchivo);
				cant++;
				i++;
...
void imprimirDatosEncolumnados(sproducto l[], int cant)
{
	...
	for(i=0; i<cant; i++)
	{
		printf("%-*s", 15, l[i].codigoBarras);
		printf("%-*s", 15, l[i].descripcion);
