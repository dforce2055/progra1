...
int altaProducto(lista *pDada, listaPaises pLista, listaTipoProductos tLista);
...
/* COMO SALIDA, EN CASO DE DARSE DE ALTA PRODUCTOS, GENERA UN NUEVO NODO EN LA LISTA DE PRODUCTOS, EN CASO DE CANCELAR LA CARGA DEVUELVE UN CERO A LA FUNCION MAIN*/
int altaProducto(lista *pDada, listaPaises pListaPaises, listaTipoProductos tListaTipoProductos)
{
	pNodo nuevo, actual;
	int salida=1 ;
	sfecha fechaActual;
			...
			else
			{
				do
				{
					if((validarCodigo(nuevo->codigoBarras))!=1)
						printf("\nERROR: El codigo de barras no es alfanumerico\n");

					if(buscarCodigoBarras(*pDada, nuevo->codigoBarras)==1)
						printf("\nERROR: El codigo del producto esta repetido\n");

					if(validarCodigoPaisEnCodigoBarras(nuevo->codigoBarras, pListaPaises)==0)
						printf("\nERROR: El codigo de pais no se encuentra en el archivo de paises\n");

					if(validarCodigoTipoProductoEnCodigoBarras(nuevo->codigoBarras, tListaTipoProductos)==0)
						printf("\nERROR: El codigo de tipo de producto no se encuentra en el archivo de codigo de tipos de productos\n");

					if( validarCodigo(nuevo->codigoBarras)!=1 || buscarCodigoBarras(*pDada, nuevo->codigoBarras)==1 ||
						validarCodigoPaisEnCodigoBarras(nuevo->codigoBarras, pListaPaises)==0 || validarCodigoTipoProductoEnCodigoBarras(nuevo->codigoBarras, tListaTipoProductos)==0)
					{
